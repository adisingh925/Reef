...
import app.android.damien.reef.widgetprovider.apexpowervaluewidget.ApexPowerValueWidgetProvider_1
...
class ApexGraphWidgetProvider_1 : AppWidgetProvider() {
        ...
        appWidgetIds?.forEach { appWidgetId ->
            ...
            CoroutineScope(Dispatchers.IO).launch {
                ...
                CoroutineScope(Dispatchers.Main).launch {
                    ...
                    if (data.lastIndex >= 0) {
                        val graph = data[0].records?.split(",")?.map { it.toFloat() }
                        ...
                        views.setTextViewText(R.id.unit, data[0].unit)
                    ...
                    val intent = Intent(context, ApexGraphWidgetProvider_1::class.java)
    ...
    override fun onReceive(context: Context?, intent: Intent?) {
        ...
        if (intent?.action == Constants.UPDATE_WIDGET_ACTION) {
            ...
            Log.d("ApexGraphWidgetProvider_1", "ApexGraphWidgetProvider_1 tapped")
    ...
    private fun updateWidget(context: Context?) {
        ...
        val appWidgetIds = appWidgetManager.getAppWidgetIds(
            ComponentName(context!!, ApexGraphWidgetProvider_1::class.java)
        )
