...
class FocustronicSingleValueType2_5 : AppWidgetProvider() {
        ...
        appWidgetIds?.forEach { appWidgetId ->
            ...
            CoroutineScope(Dispatchers.IO).launch {
                ...
                if(data.lastIndex < 4){
                    ...
                    if( data[4].givenName.isNullOrBlank()){
                        if( data[4].actualName.equals("NaN")){
                            views.setTextViewText(R.id.heading, "NaN")
                        } else {
                            views.setTextViewText(R.id.heading,  data[4].actualName)
                        }
                    } else {
                        views.setTextViewText(R.id.heading,  data[4].givenName)
                    }
                    String.format(Locale.getDefault(), "%.2f",  data[4].value)
                    views.setTextViewText(R.id.value, String.format(Locale.getDefault(), "%.2f",  data[4].value))
                    views.setTextViewText(R.id.unit,  data[4].unit.toString())
                    views.setTextViewText(R.id.timestamp, SharedPreferences.read("lastUpdatedFocustronic", ""))

                    views.setTextColor(R.id.value,  data[4].textColor)
                    views.setTextColor(R.id.heading,  data[4].textColor)
                    views.setTextColor(R.id.unit,  data[4].textColor)
                    views.setTextColor(R.id.timestamp,  data[4].textColor)
                }

                val intent = Intent(context, FocustronicSingleValueType2_5::class.java)
    ...
    private fun updateWidget(context: Context?) {
        ...
        val appWidgetIds = appWidgetManager.getAppWidgetIds(
            ComponentName(context!!, FocustronicSingleValueType2_5::class.java)
        )
