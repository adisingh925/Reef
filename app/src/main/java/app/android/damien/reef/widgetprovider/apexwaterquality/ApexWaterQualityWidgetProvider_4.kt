...
class ApexWaterQualityWidgetProvider_4 : AppWidgetProvider() {
        ...
        appWidgetIds?.forEach { appWidgetId ->
            ...
            CoroutineScope(Dispatchers.IO).launch {
                ...
                if(data.lastIndex < 3){
                    ...
                    views.setTextViewText(R.id.slot4value, data[3].slot4Value.toString())
                    views.setTextViewText(R.id.slot5value, data[3].slot5Value.toString())

                    views.setTextViewText(R.id.slot1unit, data[3].slot1Unit)
                    views.setTextViewText(R.id.slot2unit, data[3].slot2Unit)
                    views.setTextViewText(R.id.slot3unit, data[3].slot3Unit)
                    views.setTextViewText(R.id.slot4unit, data[3].slot4Unit)
                    views.setTextViewText(R.id.slot5unit, data[3].slot5Unit)
                    ...
                    if(data[3].slot4GivenName.isNullOrBlank()){
                        if(data[3].slot4ActualName.equals("NaN")){
                            views.setTextViewText(R.id.slot4name, "NaN")
                        } else {
                            views.setTextViewText(R.id.slot4name, data[3].slot4ActualName)
                        }
                    } else {
                        views.setTextViewText(R.id.slot4name, data[3].slot4GivenName)
                    }

                    if(data[3].slot5GivenName.isNullOrBlank()){
                        if(data[3].slot5ActualName.equals("NaN")){
                            views.setTextViewText(R.id.slot5name, "NaN")
                        } else {
                            views.setTextViewText(R.id.slot5name, data[3].slot5ActualName)
                        }
                    } else {
                        views.setTextViewText(R.id.slot5name, data[3].slot5GivenName)
                    }
                }

                val intent = Intent(context, ApexWaterQualityWidgetProvider_4::class.java)
    ...
    private fun updateWidget(context: Context?) {
        ...
        val appWidgetIds = appWidgetManager.getAppWidgetIds(
            ComponentName(context!!, ApexWaterQualityWidgetProvider_4::class.java)
        )
