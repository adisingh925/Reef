...
class ApexWaterQualityWidgetProvider_2 : AppWidgetProvider() {
        ...
        appWidgetIds?.forEach { appWidgetId ->
            ...
            CoroutineScope(Dispatchers.IO).launch {
                ...
                if(data.lastIndex < 1){
                    ...
                    views.setTextViewText(R.id.slot4value, data[1].slot4Value.toString())
                    views.setTextViewText(R.id.slot5value, data[1].slot5Value.toString())

                    views.setTextViewText(R.id.slot1unit, data[1].slot1Unit)
                    views.setTextViewText(R.id.slot2unit, data[1].slot2Unit)
                    views.setTextViewText(R.id.slot3unit, data[1].slot3Unit)
                    views.setTextViewText(R.id.slot4unit, data[1].slot4Unit)
                    views.setTextViewText(R.id.slot5unit, data[1].slot5Unit)
                    ...
                    if(data[1].slot4GivenName.isNullOrBlank()){
                        if(data[1].slot4ActualName.equals("NaN")){
                            views.setTextViewText(R.id.slot4name, "NaN")
                        } else {
                            views.setTextViewText(R.id.slot4name, data[1].slot4ActualName)
                        }
                    } else {
                        views.setTextViewText(R.id.slot4name, data[1].slot4GivenName)
                    }

                    if(data[1].slot5GivenName.isNullOrBlank()){
                        if(data[1].slot5ActualName.equals("NaN")){
                            views.setTextViewText(R.id.slot5name, "NaN")
                        } else {
                            views.setTextViewText(R.id.slot5name, data[1].slot5ActualName)
                        }
                    } else {
                        views.setTextViewText(R.id.slot5name, data[1].slot5GivenName)
                    }
                }

                val intent = Intent(context, ApexWaterQualityWidgetProvider_2::class.java)
    ...
    private fun updateWidget(context: Context?) {
        ...
        val appWidgetIds = appWidgetManager.getAppWidgetIds(
            ComponentName(context!!, ApexWaterQualityWidgetProvider_2::class.java)
        )
